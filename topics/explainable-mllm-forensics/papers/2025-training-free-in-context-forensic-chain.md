# Training-Free In-Context Forensic Chain for Image Manipulation Detection and Localization

## Metadata

- **Year**: 2025
- **Authors**: Rui Chen, Bin Liu, Changtao Miao, Xinghao Wang, Yi Li, Tao Gong, Qi Chu, Nenghai Yu
- **Venue**: TODO
- **Paper Link**: TODO
- **Code Link**: TODO
- **Project Page**: TODO
- **BibTeX Key**: chen2025icfc
- **Tags**: image-forensics, localization, mllm, training-free, reasoning
- **Status**: to-read
- **Added Date**: 2026-05-18
- **Last Updated**: 2026-05-18

## One-line Summary

TODO: Summarize the In-Context Forensic Chain and its training-free reasoning workflow.

## Local Source

- Local PDF: `pdfs/Explainable and MLLM-Assisted Forensics/TRAINING-FREE IN-CONTEXT FORENSIC CHAIN FOR IMAGE MANIPULATION.pdf`
- SHA-256: `0968DBDC0C6F4C06587E6873DFF18867CBAC6B96DF28C6C089AC38ED042AD195`

## Citation

```bibtex
@misc{chen2025icfc,
  title = {Training-Free In-Context Forensic Chain for Image Manipulation Detection and Localization},
  author = {Rui Chen and Bin Liu and Changtao Miao and Xinghao Wang and Yi Li and Tao Gong and Qi Chu and Nenghai Yu},
  year = {2025},
  note = {TODO: venue and link need verification}
}
```
