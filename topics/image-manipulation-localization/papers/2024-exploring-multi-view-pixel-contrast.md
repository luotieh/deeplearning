# Exploring Multi-view Pixel Contrast for General and Robust Image Forgery Localization

## Metadata

- **Year**: 2024
- **Authors**: Zijie Lou, Gang Cao, Kun Guo, Haochen Zhu, Lifang Yu
- **Venue**: TODO
- **Paper Link**: TODO
- **Code Link**: TODO
- **Project Page**: TODO
- **BibTeX Key**: lou2024multiview
- **Tags**: image-forensics, localization, contrastive-learning
- **Status**: to-read
- **Added Date**: 2026-05-18
- **Last Updated**: 2026-05-18

## One-line Summary

TODO: Summarize the multi-view pixel-wise contrastive learning strategy.

## Local Source

- Local PDF: `Image Manipulation Localization/Exploring Multi-view Pixel Contrast for General and Robust.pdf`
- SHA-256: `9810800E603D96E6CA2F18D3EC005752A96DFEC315EA63920960D77978D86F4B`

## Citation

```bibtex
@misc{lou2024multiview,
  title = {Exploring Multi-view Pixel Contrast for General and Robust Image Forgery Localization},
  author = {Zijie Lou and Gang Cao and Kun Guo and Haochen Zhu and Lifang Yu},
  year = {2024},
  note = {TODO: venue and link need verification}
}
```
